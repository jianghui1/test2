...
- (void)startRequest {
    ...
    // 设置请求类型
    if (self.requestType) {
        self.manager.requestSerializer  = [HttpRequest requestSerializerWith:self.requestType];
    } else {
        self.manager.requestSerializer  = [HttpRequest requestSerializerWith:HTTPRequestType];
    }
    
    // 设置回复类型
    if (self.responseType) {
        self.manager.responseSerializer = [HttpRequest responseSerializerWith:self.responseType];
        ...
        self.manager.responseSerializer = [HttpRequest responseSerializerWith:HTTPResponseType];
        self.manager.responseSerializer.acceptableContentTypes = \
        [self.manager.responseSerializer.acceptableContentTypes setByAddingObject:@"text/html"];
    }
    ...
    if (self.RequestMethod == GET_METHOD) {
        ...
        __weak HttpRequest *weakSelf = self;
...
+ (instancetype)networkingWithUrlString:(NSString *)urlString
    ...
    HttpRequest *networking       = [[[self class] alloc] init];
...
+ (AFHTTPRequestOperation *)GET:(NSString *)URLString
    ...
    // 设置请求类型
    manager.requestSerializer                         = [HttpRequest requestSerializerWith:requestType];
    
    
    // 设置回复类型
    manager.responseSerializer                        = [HttpRequest responseSerializerWith:responseType];
